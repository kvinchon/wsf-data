    ...
    handler: async (req, toolkit) => {
        ...
        if (req.params.user_id) {
            switch (req.params.time_period) {
                case "today":
                    subquery = db.select('id', 'date', db.raw('SUM(?? + ??) / SUM(?? + ?? + ??) * 100 AS ??', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid', 'selfconsumption'])).from('history_daily_1').where('user_id', req.params.user_id).andWhere('date', '2019-12-31').having(db.raw('SUM(?? + ?? + ??) > 0', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid'])).groupBy('id', 'date');
                    break;
                case "week":
                    subquery = db.select('id', 'date', db.raw('SUM(?? + ??) / SUM(?? + ?? + ??) * 100 AS ??', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid', 'selfconsumption'])).from('history_daily_1').where('user_id', req.params.user_id).andWhere('date', '>=', '2019-12-25').andWhere('date', '<', '2020-01-01').having(db.raw('SUM(?? + ?? + ??) > 0', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid'])).groupBy('id', 'date');
                    break;
                case "month":
                    subquery = db.select('id', 'date', db.raw('SUM(?? + ??) / SUM(?? + ?? + ??) * 100 AS ??', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid', 'selfconsumption'])).from('history_daily_1').where('user_id', req.params.user_id).andWhere('date', '>=', '2019-12-01').andWhere('date', '<', '2020-01-01').having(db.raw('SUM(?? + ?? + ??) > 0', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid'])).groupBy('id', 'date');
                    break;
                case "total":
            ...
            switch (req.params.time_period) {
                case "today":
                    subquery = db.select('id', 'date', db.raw('SUM(?? + ??) / SUM(?? + ?? + ??) * 100 AS ??', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid', 'selfconsumption'])).from('history_daily_1').where('date', '2019-12-31').having(db.raw('SUM(?? + ?? + ??) > 0', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid'])).groupBy('id', 'date');
                    break;
                case "week":
                    subquery = db.select('id', 'date', db.raw('SUM(?? + ??) / SUM(?? + ?? + ??) * 100 AS ??', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid', 'selfconsumption'])).from('history_daily_1').where('date', '>=', '2019-12-25').andWhere('date', '<', '2020-01-01 00:00:00').having(db.raw('SUM(?? + ?? + ??) > 0', ['from_gen_to_consumer', 'from_gen_to_batt', 'from_gen_to_grid'])).groupBy('id', 'date');
                    break;
                case "month":
